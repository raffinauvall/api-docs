...
export async function listApis() {
  return (
    await query(
      `SELECT a.id, a.group_id, a.name, a.slug, a.description, a.base_url, a.created_by,
  ...
  return (
    await query(
      `SELECT a.*, g.name AS group_name,
              s.id AS source_id, s.provider, s.repository, s.branch, s.file_path,
              s.last_synced_at, s.last_commit_sha, s.last_sync_status, s.last_sync_error,
              (SELECT v.version FROM api_versions v
                WHERE v.api_id = a.id AND v.is_active = true
                ORDER BY v.created_at DESC LIMIT 1) AS active_version,
...
export async function createApi(client, { groupId, name, slug, description, baseUrl, createdBy }) {
  const res = await client.query(
    `INSERT INTO apis (group_id, name, slug, description, base_url, created_by)
     VALUES ($1, $2, $3, $4, $5, $6)
     ON CONFLICT (group_id, slug) DO UPDATE SET
       ...
       updated_at = now()
     RETURNING *`,
    [groupId, name, slug, description || null, baseUrl || null, createdBy]
  )
  return res.rows[0]
}

export async function updateApiForClient(client, id, fields) {
  const keys = []
  const values = []
  let i = 1
